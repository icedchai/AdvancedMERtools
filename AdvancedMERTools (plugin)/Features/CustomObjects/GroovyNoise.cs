...
public class GroovyNoise : AMERTInteractable
{
    ...
    protected virtual void Update()
    {
        if (Active)
        {
            RandomExecutionModule.Execute(Base.Settings, new ModuleGeneralArguments { Schematic = OSchematic, Transform = transform });
        }
        Active = false;
    }
}
...
public class FGroovyNoise : GroovyNoise
{
    ...
    protected override void Update()
    {
        if (Active)
        {
            FRandomExecutionModule.Execute(Base.Settings, new FunctionArgument(this));
        }
        Active = false;
    }
}
